        ...
        <el-card class="box-card" v-for="(c, index) in CommentList" :key="index" shadow="hover"
          ...
          <el-row :gutter="2">
            ...
            <el-col :span="20" style="padding-left: .2em;">
              ...
              <span style="float: left;color: #5a5e66; font-size: small"> 评论于： {{timeStampToString(c.comment.time, "second")}}</span>
          ...
          <el-card v-if="!c.hasReply" style="width: 90%; float: right;margin-bottom:0;margin-top:0;border: none;background-color: rgba(255,255,255,0)"
                   shadow="none">
            <el-row :gutter="10" v-for="reply_id in replyCommentDict[c.comment._id]">
              <el-col :span="3" style="margin-top: 1em">
                ...
                <div style="font-family: 'PingFang SC'; font-size: .9em;">{{returnCommentById(reply_id).username}}</div>
              </el-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteThisComment(id) {
        ...
        deleteComment(id).then((res) => {
          if (res.status === 204) {
            ...
            let childList = this.getReplyList(id)
      ...
      addEmoji(emoji, kind) {
        let link_url = "<img src=\"" + emoji + "\" width=20%" + ">";
        if (kind === 'comment') {
